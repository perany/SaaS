...
function svg(option) {
    ...
    svgLib.drawXk = function(data) {
        ...
        if (option.y.length == 0) {
            if (option.ftype != 'multi') {
                ...
                ////console.log(option.y,'option.y',svgLib.el,option.avage)
        ...
        ////console.log(option.avage,'option.avage',option.maxX)
        ...
        for (var j = 0; j < ylength; j++) {
            ...
            if (option.y[j] != null && option.y[j] != undefined && option.ruleshow[1]) {
                ...
                if (yA && yB) {
                    svgLib.paper.text(option.xyshow[1] ? (stTP - 10) : 0, syt - jg * j, option.formatKdY(yB[j])).attr({
                        font: svgLib.font,
                        fill: svgLib.fontColor,
                        'width': 80,
                    ...
                    svgLib.paper.text(option.xyshow[1] ? (stTP - 10) : 0, syt - jg * j, option.formatKdY(option.y[j])).attr({
                        font: svgLib.font,
                        fill: svgLib.fontColor,
                        'width': 80,
                        'text-anchor': option.xyshow[1] ? 'start' : 'end'
    ...
    function numberXY(value) {
        ...
        if (newdata[0] * newdata[newdata.length - 1] >= 0) {
            ...
            maxL = (String(maxNUm).substr(0, 1) * 1 + 1) * Math.pow(10, (String(parseInt(maxNUm))).length - 1)
            result = [-maxL, -maxL / 2, 0, maxL / 2, maxL]
            //console.log(maxNUm,maxL,'dddddddddd',newdata)
        }
