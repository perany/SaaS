        ...
        switch (nowOption.ctype) {
            case 'normal':
                ...
                if (nowOption.yr) {
                    ...
                    for (var ww = 0; ww < nowOption.data.length; ww++) {
                        if (nowOption.data[ww]) {
                            if (ww != nowOption.data.length - 1) {
                                per.push(Math.round(100 * nowOption.data[ww] / datasum))
                            } else {
                                var num = eval(per.join('+'))
                                per.push(100 - num)
                            }
                        }
                    }
...
function drawRectOne(nowOption, newInfo, an, texShow, color, mouseEvent, zongzhi, jg) {
    ...
    for (var i = 0; i < nowOption.data.length; i++) {
        ...
        if (mouseEvent && nowOption.maxY) {
            ...
            var rec1 = nowOption.paper.rect(xPoss - 5, lindehjy, 10, hlin).attr({
                fill: '#fff',
                'stroke-width': 1,
                'fill-opacity': 0,
                'opacity': 0
        ...
        if (nowOption.yr && nowOption.maxX && nowOption.ctype != 'add') {
            /*nowOption.paper.text(nowOption.showArr.st + nowOption.showArr.w - 5, nowOption.showArr.h + nowOption.top - Math.floor(i * newInfo.jg + 0.5 * newInfo.jg), Math.round(100 * nowOption.data[i] / newInfo.datasum) + '%').attr({
                font: nowOption.font,
                fill: nowOption.fontColor,
                'text-anchor': 'end'
            })*/
             nowOption.paper.text(nowOption.showArr.st + nowOption.showArr.w - 5, nowOption.showArr.h + nowOption.top - Math.floor(i * newInfo.jg + 0.5 * newInfo.jg),  (newInfo.jiangui[i] +'%')).attr({
                font: nowOption.font,
                fill: nowOption.fontColor,
                'text-anchor': 'end'
            })
        ...
        switch (texShow) {
            case "normal":
                ...
                var txt = nowOption.paper.text(textXY.x, textXY.y, nowOption.format(nowOption.data[i], i) + nowOption.unit).attr({
                    font: nowOption.font,
            ...
            case "add":
                if (mouseEvent) {
                    ...
                    var txt = nowOption.paper.text(textXY.x, textXY.y, nowOption.format(zongzhi[i], i) + nowOption.unit).attr({
                        font: nowOption.font,
                        fill: nowOption.colorShow,
                        'text-anchor': textXY.algin
                    });
                }
                break
